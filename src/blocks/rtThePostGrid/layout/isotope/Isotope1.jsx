    ...
    return(
        <>
            <Content_wrap css={content_wrap} css_pad={constent_box_padding} className={`${parent_class}`}>
                ...
                <div className="tpg-iso-filter">
                    <div className="rt-tpg-isotope-buttons" data-url="" data-count="">
                        <button data-filter="*" className="selected">Show all</button>
                        {
                            term_cat?.length && term_cat?.map((items) =>{
                                return(
                                    <>
                                        <button className={`rt-iso-btn-${items.value}`} data-filter={`.${items.value}`}>{items.label}</button>
                                    </>
                                )
                            })
                        }
                ...
                <Content_padding css_pad={content_padding}className={`rt-row rt-content-loader layout2 tpg-even`}>
                    {
                        data.length && data.map((post) => {
                            ...
                            return(
                                <>
                                    <div
                                        className={`rt-col-md-${columns.desktop} rt-col-sm-${(columns.tablet == "24") ? "2" : columns.tablet} rt-col-xs-${(columns.mobile == "24") ? "2" : columns.mobile} rt-equal-height  rt-grid-item even-grid-item`}>
                                                    ...
                                                    general.see_more ? (
                                                        <Btn_align css_btn={button} className="post-meta ">
															<span className="read-more">
																<Button_style css={button_style} primary={primary_color}
